...
namespace CodingChallengeApi.Controllers
{
    ...
    */
    ...
    public class AppointmentController : ControllerBase
    {
        ...
        [HttpPut]
        [ActionName("Create")]
        public Response CreateAppointments([FromBody] Appointment value)
        {
            ICreateAppointment createAppointment = new CreateAppointment();
            Tuple<bool, string> res = createAppointment.Execute(value);
        ...
        {
            IGetAllAvailableAppointments getAllAvailable = new GetAllAvailableAppointments();
            List<Appointment> appointments = getAllAvailable.Execute();
            return JsonSerializer.Serialize(appointments);
        }

        [HttpPost]
        [ActionName("Update")]
        public Response UpdateAppointmentToReservePatient([FromBody] Appointment value)
        {
            IUpdateAppointmentToReservePatient updateAppointmentToReservePatient = new UpdateAppointmentToReservePatient();
            Tuple<bool, string> res = updateAppointmentToReservePatient.Execute(value);
        ...
        [HttpGet]
        [ActionName("Confirm")]
        public Response ConfirmAppointment(string patientId, string practitionerId)
        {
            IConfirmAppointments confirmAppointment = new ConfirmAppointments();
            Tuple<bool, string> res = confirmAppointment.Execute(patientId, practitionerId);
            return new Response() { Status = res.Item1.ToString(), Message = res.Item2 };
        }
    }
}
